...
import { dataCity } from "../Config/index.js";

const checkOptions = dataCity.filter(function (item) {
  return item.isCheck;
});
const checkListVbl = checkOptions.map(function (item) {
  return item.id;
});
const columnOptions = dataCity.filter(function (item) {
  return item.isColumn;
});
const selectOptions = [];
const selectData = {};
const selectValue = {};
const isSelectList = dataCity.reduce(function (vs, v) {
  if (v.isSelect) {
    vs.push(v);
...
export default {
  ...
  methods: {
    ...
    salesVolumeList(type) {
      ...
      const promise = this.authority
        ? carEnergyNewList(data)
        : energyNewList(data);
      promise.then((res) => {
        this.loading = false;
