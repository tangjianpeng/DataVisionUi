...
import { traderAll } from "../Config/index.js";

const checkOptions = traderAll.filter(function (item) {
  return item.isCheck;
});
const checkListVbl = checkOptions.map(function (item) {
  return item.id;
});
const columnOptions = traderAll.filter(function (item) {
  return item.isColumn;
});
const selectOptions = [];
const selectData = {};
const selectValue = {};
const isSelectList = traderAll.reduce(function (vs, v) {
  if (v.isSelect) {
    vs.push(v);
