...
import { stockAll } from "../Config/index.js";

const checkOptions = stockAll.filter(function (item) {
  return item.isCheck;
});
const checkListVbl = checkOptions.map(function (item) {
  return item.id;
});
const columnOptions = stockAll.filter(function (item) {
  return item.isColumn;
});
const selectOptions = [];
const selectData = {};
const selectValue = {};
const isSelectList = stockAll.reduce(function (vs, v) {
  if (v.isSelect) {
    vs.push(v);
