...
import { stockCity } from "../Config/index.js";

const checkOptions = stockCity.filter(function (item) {
  return item.isCheck;
});
const checkListVbl = checkOptions.map(function (item) {
  return item.id;
});
const columnOptions = stockCity.filter(function (item) {
  return item.isColumn;
});
const selectOptions = [];
const selectData = {};
const selectValue = {};
const isSelectList = stockCity.reduce(function (vs, v) {
  if (v.isSelect) {
    vs.push(v);
...
export default {
  ...
  mounted() {
    ...
    dateRanges().then((res) => {
      if (res.code == 200) {
        ...
        this.endDate = edata;
        this.selectValue.yearId = [edata, edata];
  ...
  methods: {
    ...
    salesVolumeList(type) {
      ...
      let data = {
        ...
        beginDate: this.selectValue.yearId[0],
        endDate: this.selectValue.yearId[1],
  ...
  watch: {
    "selectValue.provinceName": function (data, old) {
