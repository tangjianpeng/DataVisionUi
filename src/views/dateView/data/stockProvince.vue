...
import { stockProvince } from "../Config/index.js";

const checkOptions = stockProvince.filter(function (item) {
  return item.isCheck;
});
const checkListVbl = checkOptions.map(function (item) {
  return item.id;
});
const columnOptions = stockProvince.filter(function (item) {
  return item.isColumn;
});
const selectOptions = [];
const selectData = {};
const selectValue = {};
const isSelectList = stockProvince.reduce(function (vs, v) {
  if (v.isSelect) {
    vs.push(v);
...
stockProvince.map(function (item) {
  if (item.isSelect) {
    selectData[item.key] = [];
