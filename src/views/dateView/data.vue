<template>
  <div class="center_">
    <h5 class="title">全国汽车销量
      <span>全品牌乘用车，所有地级市，新车终端零售量，每月15日更新</span>
    </h5>
    <div class="selectionBox">
      ...
      <div class="checkboxBox">
        <div class="checkbox_l">
          <el-button type="primary" @click="salesVolumeList">查 询</el-button>
        </div>

        <div class="checkbox_item">
          <el-checkbox-group v-model="checkList">
            <el-checkbox label="ym_id">年月</el-checkbox>
...
<script>
import { dvIndustry, dvProvince, dvCity, dvCarBrand, salesVolumeList, volumeList, dvCarSeries } from "@/api/data";
export default {
  name: '',
  data() {
    return {
      ...
      checkList: ['ym_id', 'manf_name', 'brand_name', 'manf_prop_name', 'brand_orig_name', 'segment_name', 'sub_model_name', 'segment_full_name', 'fuel_type_name', 'vehicle_func_name', 'engine_type_name', 'tran_type_name'],
      tableData: [],
      province: [],
      cityName: [],
      carBrand: [],
  ...
  methods: {
    ...
    dvIndustry() {
      dvIndustry().then( res => {
        let data = res.data || [];
        this.itemData.beginDate = data[3].dateName || '';
        this.itemData.endDate = data[3].dateName || '';
        this.salesVolumeList();
      })
    },
    // 选择品牌
    ...
    //选择车型
    dvCarSeries(data) {
      dvCarSeries({bcode: data}).then( res => {
  ...
  computed: {
    userId: function() {
      return this.$store.getters.user.userId || '0'
    },

    startTimePicker() {
      return {
        disabledDate: time => (this.itemData.beginDate ? time.getTime() > new Date(this.form.endTime).getTime() : false),
      };
    },
    endTimePicker() {
      return {
        disabledDate: time => (this.itemData.endDate ? time.getTime() < new Date(this.form.startTime).getTime() : false),
      };
    },
