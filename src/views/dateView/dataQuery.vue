  ...
  <div class="center">
    <div class="center_l">
      ...
      <div class="itemBox_">
        ...
        <p @click="onClickData='commercialVehicle_a'" :class="color('commercialVehicle_a')">全国商用车销量</p>
        <p @click="onClickData='commercialVehicle_b'" :class="color('commercialVehicle_b')">省份商用车销量</p>
        <p @click="onClickData='commercialVehicle_c'" :class="color('commercialVehicle_c')">城市商用车销量</p>
      </div>
      <div class="itemBox_">
    ...
    <div class="center_r">
      <dataAll v-if="onClickData==='dataAll'"
      :authority="authority"
      @messageBox="dialogVisible = true"
      ></dataAll>
      <dataCity v-if="onClickData==='dataCity'"
      :authority="authority"
      @messageBox="dialogVisible = true"
      ></dataCity>
      <dataProvince v-if="onClickData==='dataProvince'"
      :authority="authority"
      @messageBox="dialogVisible = true"
      ></dataProvince>
      <newEnergyAll v-if="onClickData==='newEnergy_a'"
      :authority="authority"
      @messageBox="dialogVisible = true"
      ></newEnergyAll>
      <newEnergyCity v-if="onClickData==='newEnergy_c'"
      :authority="authority"
      @messageBox="dialogVisible = true"
      ></newEnergyCity>
      <newEnergyProvince v-if="onClickData==='newEnergy_b'"
      :authority="authority"
      @messageBox="dialogVisible = true"
      ></newEnergyProvince>
      <traderAll v-if="onClickData==='commercialVehicle_a'"
      :authority="authority"
      @messageBox="dialogVisible = true"
      ></traderAll>
      <traderProvince v-if="onClickData==='commercialVehicle_b'"
      :authority="authority"
      @messageBox="dialogVisible = true"
      ></traderProvince>
      <traderCity v-if="onClickData==='commercialVehicle_c'"
      :authority="authority"
      @messageBox="dialogVisible = true"
      ></traderCity>
      <stockAll v-if="onClickData==='ownership_a'"
      :authority="authority"
      @messageBox="dialogVisible = true"
      ></stockAll>
      <stockProvince v-if="onClickData==='ownership_b'"
      :authority="authority"
      @messageBox="dialogVisible = true"
      ></stockProvince>
      <stockCity v-if="onClickData==='ownership_c'"
      :authority="authority"
      @messageBox="dialogVisible = true"
      ></stockCity>
    </div>
    <el-dialog
      ...
      <p v-if="userId ==='0'" style="text-align: center;">请先<span style="color:red;" @click="to('login')">登陆</span>后购买数据</p>
      <p v-if="userId ==='0'" style="text-align: center;">如还没有账号请<span style="color:red;" @click="to('registered')">注册</span></p>

      <p v-if="userId !='0'" style="text-align: center;">您的权限不足，请先<span style="color:red;" @click="to('buy')">购买</span>数据</p>


      <span slot="footer" class="dialog-footer">
        <el-button type="primary" style="background-color: #24A2A1; border-color: #24A2A1;" @click="dialogVisible = false">取消</el-button>
      </span>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  mounted() {
    document.title="汽车销量查询_汽车销量数据分析_保有量数据分析等- 数觉 DataVision";
